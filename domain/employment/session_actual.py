...
def _strict_duration(value: Any) -> int:
    if type(value) is not int or value < 0:
        raise SessionActualContractError("actual_duration_minutes invalide")
    return value
...
class SessionActual:
    ...
    def from_payload(cls, payload: Mapping[str, Any]) -> "SessionActual":
        ...
        if status == "realisee":
            ...
            duration = _strict_duration(duration)
            ...
            if any(v is not None for v in (staff_uid, place_uid, start_time, end_time)) or duration not in (None, ""):
                raise SessionActualContractError("une séance annulée ne porte ni intervenant, ni lieu, ni horaires réels")
            if comment is None:
    ...
    def payload_sha256(self) -> str:
        serialized = json.dumps(self.canonical_payload(), sort_keys=True, separators=(",", ":"), ensure_ascii=True)
