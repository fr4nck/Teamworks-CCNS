...
MODERN_PAGE3 = ROOT / "teamworks" / "Ctrl" / "CTRL_Creation_contrat_p3_modern.py"
...
def test_contract_dialog_wraps_page3_decimal_quantization_in_local_context() -> None:
    ...
    modern_source = MODERN_PAGE3.read_text(encoding="utf-8")

    # La chaîne est volontairement à deux adaptateurs : le moteur métier moderne
    # enveloppe la page historique, puis le dialogue isole ses quantize dans un
    # localcontext Decimal. Aucun calcul ne doit dépendre du contexte global.
    assert 'from decimal import localcontext' in dialog_source
    assert 'from Ctrl.CTRL_Creation_contrat_p3_modern import Page as LegacyPage3' in dialog_source
    assert 'from Ctrl.CTRL_Creation_contrat_p3 import Page as LegacyPage' in modern_source
