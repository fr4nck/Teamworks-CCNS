...
def test_structural_diff_separates_cosmetic_business_style_asset_placeholder_and_structure_changes():
    ...
    asset_a = base.replace(b"<text>{NOM} Bonjour</text>", b"<image imagetype=\"1\"><data>424D0001</data></image>")
    asset_b = base.replace(b"<text>{NOM} Bonjour</text>", b"<image imagetype=\"1\"><data>424D0002</data></image>")
    asset_diff = compare_twd_bytes(asset_a, asset_b, name_a="asset-a", name_b="asset-b")
    assert asset_diff.asset_changes
...
def test_inventory_reports_unknown_element_and_attribute_without_gui_dependency():
    ...
    imported = import_twd_bytes(raw, source="future.twd")
    assert "paragraphlayout-attribute:futureattr" in imported.unsupported_features
    assert "paragraph-element:future" in imported.unsupported_features


def test_importer_reports_unexpected_richtext_version_without_silent_acceptance():
    raw = b'''<?xml version="1.0" encoding="UTF-8"?>\n<richtext version="9.9" xmlns="http://www.wxwidgets.org"><paragraphlayout><paragraph><text>version future</text></paragraph></paragraphlayout></richtext>'''
    result = import_twd_bytes(raw, source="future-version.twd")
    assert "richtext-version:9.9" in result.unsupported_features
    assert any("Version wxRichText non qualifiée" in warning for warning in result.warnings)
