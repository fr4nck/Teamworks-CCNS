...
def test_boutons_tableau_de_bord_utilisent_les_filtres_existants_sans_reconstruction():
    ...
    assert 'self.button_show_non_compliant = CTRL_Bouton_image.CTRL(' in source
    assert 'texte="Voir les non conformes"' in source
    assert 'self.button_show_not_evaluated = CTRL_Bouton_image.CTRL(' in source
    assert 'texte="Voir les non évaluables"' in source
