...
L'espace Recrutement global est distinct de l'onglet Recrutement de la fiche individuelle. Sa coque Qt reprend la composition historique de `CTRL_Recrutement.py` :

- splitter vertical avec colonne de suivi à gauche et contenu principal à droite ;
- `Prochains entretiens`, puis `À traiter` avec la description historique ;
- quatre modes distincts : Candidats, Candidatures, Entretiens, Offres d'emploi ;
...
- candidat/personne : Identité, Candidatures, Entretiens ;
- offre d'emploi : Candidatures uniquement.

La sélection utilise `recruitment_selection.py`, qui sépare la ligne cible des actions du sujet affiché dans le résumé. Aucun candidat n'est implicitement converti en personne et aucune ligne métier fictive n'est injectée. La coque est exposée séparément par `run_windows.cmd recruitment`. Les données Recrutement et toutes les écritures restent non raccordées à ce stade.
