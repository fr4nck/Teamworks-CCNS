...
_SCHEMA_STATEMENTS = (
    ...
    CREATE TABLE IF NOT EXISTS tw_hr_case_expected_documents (
        ...
        document_label VARCHAR(240) NOT NULL,
...
_INDEXES = (
    ...
    (
        ...
        "structure_ref, organization_code, status",
    ),
    (
        "tw_hr_audit_events",
...
class DuplicateTeamworksHrAuditEventError(ValueError):
    """Un événement append-only existe déjà avec le même identifiant."""


class TeamworksHrCasesRepository:
    """Persistance de production des démarches RH et de leur journal.

    Le repository utilise ``GestionDB`` par défaut afin de conserver les deux
    chemins historiques de Teamworks : SQLite local et MySQL/MariaDB réseau.
    Le schéma est strictement additif et ne crée aucune clé étrangère vers les
    personnes, contrats ou profils d'organismes historiques.

    Les dossiers sont des projections courantes modifiables. Le journal d'audit,
    lui, est append-only : un ``event_id`` déjà utilisé n'est jamais écrasé.
    """
    ...
    def save_case(self, *, structure_ref: str, case: HrCase) -> HrCase:
        structure_ref = _required_text(
            structure_ref, "La référence de structure est obligatoire."
        )
        if not isinstance(case, HrCase):
            raise TypeError("Le dossier RH à persister est invalide.")

        values = _case_values(structure_ref, case)
    ...
    def get_case(self, *, structure_ref: str, case_id: str) -> HrCase | None:
        ...
        try:
            ...
            _close(db)
        return _case_from_row(row, documents)
    ...
    def list_cases(self, *, structure_ref: str) -> tuple[HrCase, ...]:
        ...
        return tuple(
            _case_from_row(row, documents_by_case.get(row[1], ())) for row in rows
        )
    ...
    def get_event(
        self, *, structure_ref: str, event_id: str
    ) -> HrAuditEvent | None:
        structure_ref = _required_text(
            structure_ref, "La référence de structure est obligatoire."
        )
        event_id = _required_text(
            event_id, "L'identifiant de l'événement RH est obligatoire."
        )
        db = self._db_factory()
        try:
            row = _fetchone(
                db,
                "SELECT "
                + ", ".join(_AUDIT_EVENT_COLUMNS)
                + " FROM tw_hr_audit_events "
                "WHERE structure_ref = ? AND event_id = ?",
            ...
            _close(db)
        return _event_from_row(row, fields)
        ...
        *,
        ...
        if target_ref is not None:
            target_ref = _required_text(
                target_ref, "La référence de la cible d'audit est obligatoire."
            )
        ...
        return tuple(
            _event_from_row(row, fields_by_event.get(row[1], ())) for row in rows
        )
...
def _event_from_row(row, fields) -> HrAuditEvent:
    return HrAuditEvent.create(
        ...
        actor_ref=row[6],
        source=row[7],
        fields=(HrAuditField.create(key=item[0], value=item[1]) for item in fields),
    )
