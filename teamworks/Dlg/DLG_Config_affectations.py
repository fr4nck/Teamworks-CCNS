#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Référentiel moderne des affectations utilisées par le recrutement."""

import wx
import GestionDB
from Ctrl import CTRL_Bouton_image, CTRL_Section
from Dlg import DLG_Config_affectations_core as CORE
...
class ListCtrl(wx.ListCtrl):
    ...
    def MAJListeCtrl(self):
        self.DeleteAllItems()
        DB = GestionDB.DB()
        req = """SELECT affectations.IDaffectation, affectations.affectation, COUNT(cand_affectations.IDcand_affectation)
        FROM affectations LEFT JOIN cand_affectations ON cand_affectations.IDaffectation=affectations.IDaffectation
        GROUP BY affectations.IDaffectation, affectations.affectation ORDER BY affectations.affectation;"""
        DB.ExecuterReq(req); rows = DB.ResultatReq(); DB.Close()
        for index, (IDaffectation, affectation, nombre) in enumerate(rows):
            self.InsertItem(index, str(IDaffectation)); self.SetItem(index, 1, affectation or ""); self.SetItem(index, 2, str(nombre or 0))
    ...
    def _ajuster_colonnes(self):
        ...
        self.SetColumnWidth(0, 0); self.SetColumnWidth(2, max(100, int(largeur * 0.22)))
    ...
    def OnContextMenu(self, event):
        menu = wx.Menu(); id_add = wx.NewIdRef(); menu.Append(id_add, _(u"Ajouter une affectation")); self.Bind(wx.EVT_MENU, lambda evt: self.parent.Ajouter(), id=id_add)
        if self.GetFirstSelected() != -1:
            menu.AppendSeparator(); id_edit = wx.NewIdRef(); id_delete = wx.NewIdRef(); menu.Append(id_edit, _(u"Modifier")); menu.Append(id_delete, _(u"Supprimer"))
            self.Bind(wx.EVT_MENU, lambda evt: self.parent.Modifier(), id=id_edit); self.Bind(wx.EVT_MENU, lambda evt: self.parent.Supprimer(), id=id_delete)
...
class Dialog(wx.Dialog):
    def __init__(self, parent):
        wx.Dialog.__init__(self, parent, -1, style=wx.DEFAULT_DIALOG_STYLE | wx.RESIZE_BORDER); self.parent = parent; self.panel_contenu = Panel(self)
        self.bouton_aide = CTRL_Bouton_image.CTRL(self, texte=_(u"Aide")); self.bouton_fermer = CTRL_Bouton_image.CTRL(self, texte=_(u"Fermer")); self.SetTitle(_(u"Gestion des affectations"))
        padding = UTILS_Styles.GetLayoutSpacing("dialog_padding"); actions = wx.BoxSizer(wx.HORIZONTAL); actions.Add(self.bouton_aide, 0); actions.AddStretchSpacer(1); actions.Add(self.bouton_fermer, 0)
        s = wx.BoxSizer(wx.VERTICAL); s.Add(self.panel_contenu, 1, wx.EXPAND | wx.ALL, padding); s.Add(actions, 0, wx.EXPAND | wx.LEFT | wx.RIGHT | wx.BOTTOM, padding); self.SetSizer(s)
