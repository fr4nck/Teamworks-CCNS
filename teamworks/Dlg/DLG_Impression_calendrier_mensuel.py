...
class MyDialog(wx.Dialog):
    ...
    def __init__(self, parent, IDpersonne=None, annee=None, mois=None, autoriser_choix_personne=True):
        ...
        self.label_nom = wx.StaticText(self.staticbox1, -1, _(u"Mois :"))
        self.ctrl_mois = MyChoice(self.staticbox1)
        if mois == None : mois = datetime.date.today().month
        self.ctrl_mois.SetSelection(mois-1)
        self.label_annee = wx.StaticText(self.staticbox1, -1, _(u"Année :"))
        self.ctrl_annee = wx.SpinCtrl(self.staticbox1, -1, "", size=(60, -1))
        self.ctrl_annee.SetRange(1970, 2099)
        if annee == None : annee = datetime.date.today().year
        self.ctrl_annee.SetValue(annee)
        self.ctrl_personnes = MyCheckListBox(self.staticbox1, size=(-1, 90))
        
        self.staticbox2 = wx.StaticBox(self, -1, _(u"Options d'affichage"))
        self.ctrl_afficher_we = wx.CheckBox(self.staticbox2, -1, _(u"Activer la coloration des week-ends"))
        self.ctrl_afficher_vacances = wx.CheckBox(self.staticbox2, -1, _(u"Activer la coloration des vacances"))
        self.ctrl_afficher_feries = wx.CheckBox(self.staticbox2, -1, _(u"Activer la coloration des jours fériés"))
        self.ctrl_afficher_heures = wx.CheckBox(self.staticbox2, -1, _(u"Afficher le total d'heures journalier"))
        self.ctrl_afficher_heures_mois = wx.CheckBox(self.staticbox2, -1, _(u"Afficher le total d'heures mensuel"))
        self.ctrl_afficher_couleurs_categories = wx.CheckBox(self.staticbox2, -1, _(u"Activer la coloration des catégories de présence"))
        self.ctrl_afficher_legende = wx.CheckBox(self.staticbox2, -1, _(u"Afficher la légende des couleurs"))
