...
class Panel(CORE.Panel):
    def __init__(self, parent, ID=-1):
        ...
        self.section = CTRL_Section.Section(self, titre=_(u"Diffuseurs d'offres"), niveau=2, description=_(u"Organismes ou canaux utilisés pour diffuser les offres d'emploi.")); self.barreTitre = self.section.titre; contenu = self.section.GetContentPanel(); self.listCtrl = ListCtrl(contenu)
        ...
        self.Bind(wx.EVT_BUTTON, self.OnBoutonAjouter, self.bouton_ajouter); self.Bind(wx.EVT_BUTTON, self.OnBoutonModifier, self.bouton_modifier); self.Bind(wx.EVT_BUTTON, self.OnBoutonSupprimer, self.bouton_supprimer); self.Bind(wx.EVT_BUTTON, self.OnBoutonAide, self.bouton_aide); self.bouton_modifier.Enable(False); self.bouton_supprimer.Enable(False)
