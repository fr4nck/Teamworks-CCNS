...
class Page(wx.Panel):
    """Page de caractéristiques contractuelles post-TW-189.

    Le métier TW-184 est conservé, mais chaque contrôle placé dans un
    ``StaticBoxSizer`` est enfant du ``StaticBox`` correspondant. Ce parentage
    est obligatoire sous wxPython Phoenix et conserve le correctif TW-189.
    """
    ...
    def __init__(self, *args, **kwds):
        ...
        caract = self.sizer_caract_staticbox
        self.label_convention = wx.StaticText(caract, -1, _(u"Convention applicable :"))
        self.choice_convention = wx.Choice(caract, -1, choices=[])
        self._InitConventionChoice()
        self.convention_spacer = wx.StaticText(caract, -1, "")
        ...
        self.ccns_group_spacer = wx.StaticText(caract, -1, "")

        self.label_cee_qualification = wx.StaticText(caract, -1, _(u"Qualification / statut CEE :"))
        self.choice_cee_qualification = wx.Choice(caract, -1, choices=[])
        for code, label in CEE_QUALIFICATIONS:
            self.choice_cee_qualification.Append(label, code)
        self.cee_qualification_spacer = wx.StaticText(caract, -1, "")
        ...
        self.weekly_hours = wx.SpinCtrlDouble(ccns, -1, min=0.0, max=80.0, initial=35.0, inc=0.25)
        self.weekly_hours.SetDigits(2)
        self.label_weekly_unit = wx.StaticText(ccns, -1, _(u"h / semaine"))

        self.label_monthly_salary = wx.StaticText(ccns, -1, _(u"Rémunération brute :"))
        self.monthly_salary = wx.TextCtrl(ccns, -1, "")
        self.label_monthly_unit = wx.StaticText(ccns, -1, _(u"€ brut / mois"))
        self.label_ccns_preview = wx.StaticText(ccns, -1, "")
        ...
        self.datepicker_date_debut = DatePickerCtrl(dates, -1, style=DP_DROPDOWN)
        self.label_date_fin = wx.StaticText(dates, -1, _(u"Jusqu'au :"))
        self.datepicker_date_fin = DatePickerCtrl(dates, -1, style=DP_DROPDOWN)
        self.datepicker_date_debut.Enable(False)
        self.datepicker_date_fin.Enable(False)
        self.check_rupture = wx.CheckBox(dates, -1, _(u"Rupture anticipée du contrat au :"))
        self.datepicker_rupture = DatePickerCtrl(dates, -1, style=DP_DROPDOWN)
        self.datepicker_rupture.Enable(False)

        essai_box = self.sizer_essai_staticbox
        self.label_essai = wx.StaticText(essai_box, -1, _(u"Nombre de jours :"))
        self.periode_essai = wx.SpinCtrl(essai_box, -1, "", size=(70, -1))
    ...
    def RefreshCEEPreview(self):
        ...
        status = _(u"CONFORME") if employer >= legal else _(u"NON CONFORME")
        self.label_cee_preview.SetLabel(
