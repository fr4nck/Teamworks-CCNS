...
class ObjectListView(OLV.ObjectListView):
    ...
    def _HandleSize(self, evt):
        ...
        if self.HasFlag(wx.LC_NO_HEADER) :
            ...
            proportion = 2
        try :
            self.stEmptyListMsg.SetSize(0, int(int(sz.GetHeight() / proportion)), int(int(sz.GetWidth())), int(sz.GetHeight())) # J'ai mis 2 a la place de 3
        except :
            self.stEmptyListMsg.SetDimensions(0, int(int(sz.GetHeight() / proportion)), int(int(sz.GetWidth())), int(sz.GetHeight()))  # J'ai mis 2 a la place de 3
        #self.stEmptyListMsg.Wrap(int(sz.GetWidth()))
