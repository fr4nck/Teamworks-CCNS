...
tailleFontIntitule = 7              # Taille de font du texte intitul  d'une barre

couleurFontHeures = (0, 0, 0)       # Couleur de la font des heures
couleurFontIntitule = (0, 0, 0)   # Couleur de la font des intitul s
     # Couleur du texte d'un entete de ligne s lectionn 

couleurBordDefaut = (0, 0, 0)       # Couleur par d faut du bord d'une barre
couleurBackground = (255, 255, 255) # Couleur du background du DC

modeAffichage = 2   # Mode d'affichage : Dates / personnes / etc...

# Valeurs   ne pas changer :
varScroll = 0
...
largeurFinLigne = 38                # Largeur de l'espace laiss    la fin de chaque ligne pour le temps de la journ e
coordLigne = (0, 0)                 # Coordonn es XG et XD d'une ligne
selectionsLignes = []                         # Liste des s lections d'entetes de lignes
...
couleurBordSelect = (55, 228, 9)    # Couleur du bord de la case si celle-ci est s lectionn e
couleurBordSurvol = (30, 60, 255)   # Couleur du bord de la case si celle-ci est survol e
...
couleurLigneSelect = (255, 162, 0)                         # Couleur d'une ligne s lectionn e
couleurTxtLigneSelect = (255, 162, 0)                     # Couleur de txt d'un entete de ligne s lectionn 
...
def HeuresEnCoords(heure):
    """ Transforme une heure donn e en coordonn es pixels """
    ...
    n = ((HMax - HMin).seconds)/60.0/5.0 # Soit n cr neaux de 5 minutes
...
def CoordEnHeures(coordX):
    """ Transforme des coordonn es pixels en Heures """
    global heureMin, heureMax
    HMin = datetime.timedelta(hours=heureMin.hour, minutes=heureMin.minute)
    HMax = datetime.timedelta(hours=heureMax.hour, minutes=heureMax.minute)
    n = ((HMax - HMin).seconds)/60.0/5 # Soit n cr neaux de 5 minutes
    gradMin = coordLigne[0] 
    gradMax = coordLigne[1]
    EcartGrad = gradMax - gradMin
    PixelsPar5Mins = EcartGrad / n
...
def AroundHeure(heure):
    """ Arrondit une heure datetime.time donn e aux 5 minutes inf rieures ou sup rieures """
    ...
    def DrawGraduations(self, dc, heureMi="7:00", heureMa="23:30", posY=0):
        ...
        for etape in range(nbreGraduations):
            ...
            if k == 0 :
                ...
                # Dessin du texte
                dc.DrawText(texte, int(i - (largTexte / 2)), int(posY))
            # Dessin du trait
            dc.DrawLine(int(i), int(posY+hautTexte+hautTraitHeures-hauteurTrait), int(i), int(posY+hautTexte+hautTraitHeures))
            i = i + step
            j = j + 1
            k = k + 1
...
class WidgetPlanning(wx.ScrolledWindow):
    ...
    def __init__(self, parent, listePresences, dictCategories, dictGroupes, dictLignes):
        ...
        # R cup ration des p riodes de vacances et des jours f ri s
    ...
    def Init_valeurs_defaut(self):
        ...
        # D finir amplitude horaire affich e par d faut
        global heureMin, heureMax
    ...
    def DoDrawing(self, dc):
        ...
        # Dessine le Nbre de pr sents par tranche d'heures
    ...
    def Impression(self, afficher=True):
        if len(self.listePresences) == 0 :
            dlg = wx.MessageDialog(self, _(u"Vous n'avez s lectionn  aucune pr sence   imprimer !"), _(u"Erreur"), wx.OK | wx.ICON_ERROR)
            dlg.ShowModal()
            dlg.Destroy()
            return False
        
        # Met   jour les donn es du planning   partir du panel pr sences
        self.GetGrandParent().GetParent().MAJpanelPlanning()
        
        # Demande le type d'impression   l'utilisateur
    ...
    def InitPlanning(self, listePresences, dictCategories, dictGroupes, dictLignes) :
        ...
        for item in self.listePresences :
            ...
            # Cr ation du dict des IDpr sences par ligne
    ...
    def InitListePresences(self, listePresences):
        """ Cr e une liste des pr sences avec leurs coordonn es X """
        global heureMin, heureMax
        newListe = []
        for item in listePresences :
            ...
            try:
                ...
                # Ajuste les graduations en fonction des horaires donn s
    ...
    def DrawGroupes(self, dc, tailleDC, mode="screen"):
        ...
        for key, valeurs in self.dictGroupes.items() :
            ...
            # D termine la couleur de fond du groupe
    ...
    def DessineNbrePresents(self, dc, keyGroupe, IDobjet=None, refresh=False, mode="screen"):
        # Calcul du nbre de pr sents durant la journ e
        listeBarres = []
        listePositions = []
        for ligne in self.dictGroupes[keyGroupe][4] :
            keyLigne = (ligne[0], ligne[1])
        ...
        # Traitement de la liste pour trouver les nbre de pr sents par tranche horaire
        listePositions.sort()
...
##        print x, y
        ...
        if len(listeObjets) != 0 :
            ...
            if self.dictIDs[IDobjet][0] == "tache" :
                ...
                # Recherche des contraintes de la barre s lectionn e
    ...
    def HitTestBarre(self, x, IDpresence, IDobjet):
        ...
        # Recherche la position cliqu e dans la barre
    ...
    def HitTestTitreGroupe(self, x, y) :
        """ Pour s lectionner toutes les lignes d'un groupe """
    ...
    def OnDLeftDown(self, event):
        """ Double-click bouton gauche -> ajout d'une pr sence """
        x,y = self.ConvertEventCoords(event)
    ...
    def OnLeftUp(self, event):
        if self.selectModif == True :
            ...
            # MAJ du listCtrl des dur es par cat gorie
    ...
    def CreateTexteStatusBar(self):
        """ Ecrit du texte dans la barre de status de la fen tre parente """
        ...
        try : 
            ...
            txtAide = _(u"Acc dez aux fonctions de gestion des t ches en cliquant sur le bouton droit de votre souris")
...
##        self.SetFocus()
        ...
        if event.Dragging() and event.LeftIsDown() and self.selectBarre != None:
            ...
            # MAJ de la barre du nbre de pr sents
            if afficher_nbre_presents == True :
            ...
            """ Affiche dans la statusBar de la barre survol e """
            x,y = self.ConvertEventCoords(event)
            listeObjets = self.pdc.FindObjectsByBBox(x, y)
            if len(listeObjets) != 0 :
                IDobjet = listeObjets[0]
    ...
    def MoveBarre(self, x):
        """ Fonction de d placement d'une barre """
        IDpresence = self.selectBarre[0]
        ...
        if region == "milieu" :
            ...
            if heureD > self.contraintes[3] : 
                ...
                self.WarpPointer(xMouse, yMouse)
                return
            # M morisation des nouvelles valeurs
            self.dictPresences[IDpresence][3] = heureG
    ...
    def ContraintesBarre(self, IDpresence):
        """ D finit les contraintes de d placement de la barre d plac e quand elle est cliqu e pour la 1ere fois"""
        ...
        # Cr ation d'une liste des pr sences du m me jour et du m me animateur
        listeHeuresDebut = []
        listeHeuresFin = []
        for key, valeurs in self.dictPresences.items() :
        ...
        # Recherche de l'heure Max du cot  droit
        heureDMax = heureMax
    ...
    def AdapteTexteHeure(self, dc, texte, tailleBarre):
        """ Raccourcit le texte de l'heure en fonction de la taille donn e """
    ...
    def SauvegardeModif(self):
        """ Sauvegarde des donn es modifi es dans la base de donn es """
    ...
    def Ajouter(self, selection):
        """ Ajouter une pr sence avec le form de saisie des pr sences """
    ...
    def Modifier(self, IDpresence):
        """ Modifier une pr sence avec le form de saisie des pr sences """
    ...
    def Supprimer(self, IDpresence, txt):
        """ Supprimer une t che """
        dlg = wx.MessageDialog(self, txt,  _(u"Suppression d'une t che"), wx.ICON_QUESTION | wx.YES_NO | wx.NO_DEFAULT)
    ...
    def OnContextMenu(self, event):
        ...
        if self.ligne != None :
            if IDpresence == None  : 
                ...
                menu.Append(id, _(u"Enregistrer une t che unique ici"))
                ...
                # Menu Modifier la l gende
                if self.dictPresences[IDpresence][6] == "" :
                    texte3 = _(u"Ajouter une l gende")
                else:
                    texte3 = _(u"Modifier la l gende")
                id = wx.Window.NewControlId()
                self.Bind(wx.EVT_MENU, self.OnMenuModifLegende, id=id)
                menu.Append(id, texte3)
           
                # Cr ation du sous-menu Cat gories
                self.SousMenuCategories(0, menu, _(u"Changer de cat gorie"), None)
                
            menu.AppendSeparator()
            
        # Gestion des saisie de pr sences et des s lections
        if len(selectionsLignes) != 0 :
            
            # Menu Saisir une t che pour les lignes s lectionn es
            id = wx.Window.NewControlId()
            self.Bind(wx.EVT_MENU, self.OnMenuSaisie, id=id)
            if len(selectionsLignes) == 1:
                txt = _(u"Enregistrer une t che pour la ligne s lectionn e")
            else:
                txt = _(u"Enregistrer une t che pour les ") + str(len(selectionsLignes)) + _(u" lignes s lectionn es")
            menu.Append(id, txt)
            
            # Menu Supprimer toutes les t ches des des lignes s lectionn es
            id = wx.Window.NewControlId()
            self.Bind(wx.EVT_MENU, self.OnMenuSupprimerLignes, id=id)
            if len(selectionsLignes) == 1:
                txt = _(u"Supprimer toutes les t ches de la ligne s lectionn e")
            else:
                txt = _(u"Supprimer toutes les t ches des ") + str(len(selectionsLignes)) + _(u" lignes s lectionn es")
            menu.Append(id, txt)
            
            menu.AppendSeparator()
            
            # Menu Appliquer un mod le
            id = wx.Window.NewControlId()
            self.Bind(wx.EVT_MENU, self.OnMenuAppliquerModele, id=id)
            if len(selectionsLignes) == 1:
                txt = _(u"Appliquer un mod le   la ligne s lectionn e")
            else:
                txt = _(u"Appliquer un mod le aux ") + str(len(selectionsLignes)) + _(u" lignes s lectionn es")
            menu.Append(id, txt)
        ...
        menu.Append(id, _(u"S lectionner toutes les lignes"))
            
        # Menu Tout d s lectionner
        id = wx.Window.NewControlId()
        self.Bind(wx.EVT_MENU, self.OnMenuDeselectAll, id=id)
        menu.Append(id, _(u"D s lectionner toutes les lignes"))
    ...
    def OnMenuSupprimer(self, event):
        ...
        txt = _(u"Souhaitez-vous vraiment supprimer la t che du %s de %s   %s ?") % (date, heureDebut, heureFin)
        self.Supprimer(IDpresence, txt)

    def OnMenuModifLegende(self, event):
        """ Ajouter ou modifier une l gende """
        IDpresence = self.Context[0]
        legendeDefaut = self.dictPresences[IDpresence][6]
        heureDebut = str(self.dictPresences[IDpresence][3])[:5]
        heureFin = str(self.dictPresences[IDpresence][4])[:5]
        date = DatetimeDateEnStr(self.dictPresences[IDpresence][2])
        intro = _(u"Saisissez une l gende pour la t che du %s de %s   %s :") % (date, heureDebut, heureFin)
    ...
    def OnMenuAppliquerModele(self, event):
        """ Appliquer un mod le aux lignes s lectionn es """
        selectionPersonnes = []
        selectionDates = []
        ...
        for key, valeurs in self.dictCategories.items() :
            if valeurs[1] == IDparent:
                ...
                if enfantsExists == False :
                    # Si pas d'enfants, on cr e un item dans le menu
                    nbre += 1
                    ...
                    # Si un ou plusieurs enfants existente, on cr e un sous-menu
    ...
    def SaisiePresences(self):
        """ Saisie de pr sences   partir des lignes s lectionn es """ 
    ...
    def SupprimerPresencesLignes(self):
        """ Supprime toutes les t ches des lignes s lectionn es """
        nbreLignes = len(selectionsLignes)
        
        # Recherche des ID des pr sences contenues dans les lignes s lectionn es
        listeIDpresences = []
        for ligne in selectionsLignes :
            IDpersonne1 = ligne[0]
            date1 = ligne[1]
        ...
        if len(listeIDpresences) == 0 : 
            dlg = wx.MessageDialog(self, _(u"Il n'existe aucune t che   supprimer sur la ou les lignes s lectionn es !"), "Erreur", wx.OK)  
            dlg.ShowModal()
            dlg.Destroy() 
            return
        
        if nbreLignes == 1 :
            txt = _(u"Souhaitez-vous vraiment supprimer toutes les t ches de la ligne s lectionn e ?")
        else:
            txt = _(u"Souhaitez-vous vraiment supprimer toutes les t ches des ") + str(nbreLignes) + _(u" lignes s lectionn es ?")
        dlg = wx.MessageDialog(self, txt ,  _(u"Suppression de toutes les t ches"), wx.ICON_QUESTION | wx.YES_NO | wx.NO_DEFAULT)
        if dlg.ShowModal() == wx.ID_NO :
    ...
    def MAJafterModif(self):
        """ Met   jour l'affichage du DC suite   une MODIF ou un AJOUT de pr sences """
...
class BarreOptions(wx.Panel):
    ...
    def Menu_Outils(self, event):
        ...
        # Commande Imprimer le planning affich 
        IDitem = 10
        item = wx.MenuItem(menu, IDitem, _(u"Imprimer le planning affich "), _(u"Imprimer le planning affich "))
        ...
        # Commande Envoyer par email le planning affich 
        IDitem = 11
        item = wx.MenuItem(menu, IDitem, _(u"Envoyer par Email le planning affich "), _(u"Envoyer par Email le planning affich "))
        ...
        IDitem = 20
        item = wx.MenuItem(menu, IDitem, _(u"Statistiques"), _(u"Afficher les statistiques des pr sences"))
        ...
        # Commande Sc narios
        IDitem = 30
        item = wx.MenuItem(menu, IDitem, _(u"Gestion des sc narios"), _(u"Gestion des sc narios"))
    ...
    def Menu_Options(self, event):
        ...
        IDitem = 210
        menu.Append(IDitem, _(u"Afficher les l gendes"), _(u"Affiche ou non les l gendes des pr sences"), wx.ITEM_CHECK)
        if hauteurBarre == 26 :
            menu.Check(IDitem, True)
        self.Bind(wx.EVT_MENU, self.Menu_210, id=IDitem)
        
        # Affichage des p riodes de contrats
        IDitem = 220
        menu.Append(IDitem, _(u"Afficher les p riodes de contrats"), _(u"Affiche ou non les p riodes des contrats des personnes s lectionn es"), wx.ITEM_CHECK)
        if afficher_contrats == True :
            menu.Check(IDitem, True)
        ...
        IDitem = 240
        menu.Append(IDitem, _(u"Afficher les temps en t te de groupe"), _(u"Affiche ou non les temps en t te de groupe"), wx.ITEM_CHECK)
        if afficher_temps_groupe == True :
            menu.Check(IDitem, True)
        self.Bind(wx.EVT_MENU, self.Menu_240, id=IDitem)
        
        # Affichage du nbre de pr sents
        IDitem = 250
        ...
        # Commande Imprimer
        IDitem = 260
        item = wx.MenuItem(menu, IDitem, _(u"D finir l'amplitude horaire affich e par d faut"), _(u"D finir l'amplitude horaire affich e par d faut"))
        menu.AppendItem(item)
    ...
    def Menu_Mail_planning_affiche(self, event):
        ...
        # Envoyer donn es vers l' diteur d'emails
    ...
    def Menu_20(self, event):
        ...
        panelPresences = self.GetGrandParent().GetParent()
        # R cup ration des dates du calendrier
        ...
        # R cup ration des personnes de la liste de personnes
...
class PanelPlanning(wx.Panel):
    def __init__(self, parent, ID=-1):
        ...
        # Cr ation du DC des graduations
        self.DCgraduations = Graduations(self)

        # Cr ation du DC Planning
    ...
    def ImportPresences(self, listeDates, listePersonnes):
        """ R cup ration des pr sences dans la base """

        # Cr ation de la liste des conditions de dates
        strDates = ""
        for date in listeDates :
            strDates += "date='" + date + "' Or "
        strDates = strDates[:-4]
        # Cr ation de la liste des conditions de IDpersonne
        strPersonnes = ""
        for IDpersonne in listePersonnes :
            strPersonnes += "IDpersonne=" + str(IDpersonne) + " Or "
        strPersonnes = strPersonnes[:-4]
        # Cr ation du str final de conditions
    ...
    def RecherchePresents(self, listeDates):
        """ Recherche les ID des personnes pr sentes sur la p riode de dates donn es """
    ...
    def ImportGroupes_Mode1(self, listeIDPersonne, listeDates):
        """ R cup ration des pr sences dans la base
        > MODE 1
        > Groupe = DATES | Lignes = PERSONNES | Uniquement les pr sents chaque jour
        """
        print("mode 1")
        listeConditions = [str(date) for date in listeDates] 
        if len(listeConditions) == 0 : listeConditions = "('3000-10-10')"
        ...
        # Cl ture le dernier groupe
        dictGroupes[IDGroupe][2] = posY
        dictGroupes[IDGroupe][3] = nbreLignesGroupe

        # Cr ation du dictionnaire des lignes
        dictLignes = self.CreationDictLignes(dictGroupes)

        # Cr ation d'une liste temporaire des dates au format "2008-01-15"
        listeDatesTemp = []
        for date in listeDates :
            listeDatesTemp.append(str(date))

        # Cr ation de la liste des pr sences
        listePresences = []
        if len(dictLignes) != 0 :
    ...
    def ImportGroupes_Mode2(self, listeIDPersonne, listeDates):
        """ R cup ration des pr sences dans la base
        > MODE 2
        > Groupe = DATES | Lignes = PERSONNES | Toutes les personnes demand es chaque jour demand 
        """

        # Importation des noms des personnes pour les entetes de lignes
        self.dictPersonnes = self.ImportPersonnes()

        # Cr ation de la liste des personnes
        listePersonnes = []
    ...
    def ImportGroupes_Mode3(self, listeIDPersonne, listeDates):
        """ R cup ration des pr sences dans la base
        > MODE 3
        > Groupe = PERSONNES | Lignes = DATES | Toutes les personnes demand es chaque jour demand 
        """
    ...
    def CreationDictLignes(self, dictGroupes):
        """ Cr  e un dict des lignes   partir du dictGroupes """
        dictLignes = {}
        for IDgroupe, groupe in dictGroupes.items() :
            for ligne in groupe[4] :
                ...
                posYLigne = ligne[2]
                texteLigne = ligne[3]
                dictLignes[(IDpersonne, datetimeDate)] = [IDgroupe, posYLigne, texteLigne, False]       # [IDgroupe, posYligne, texte, S lectionn ]
    ...
    def ImportPersonnes(self):
        """ R cup ration des noms des personnes """
        DB = GestionDB.DB()
    ...
    def ImportCategories(self):
        """ R cup ration des cat gories de pr sences dans la base """
        DB = GestionDB.DB()
...
class ImpressionPDFvTexte():
    def __init__(self, nom_doc, dictCategories, dictGroupes, dictLignes, listePresences, dictPresences, maxWidth, maxHeight) :
        if len(dictGroupes) == 0 :
            dlg = wx.MessageDialog(None, _(u"Vous devez s lectionner au moins une date dans le calendrier !"), "Erreur", wx.OK)  
            dlg.ShowModal()
            dlg.Destroy() 
            return
        ...
        # Tri de la liste des pr sences par heure de d but
        ...
        style = TableStyle([('GRID', (0,0), (-1,-2), 0.25, colors.black),
                            ...
                            ('ALIGN', (0,1), (0,-1), 'RIGHT'), # Colonne dates align e   droite
                            ('ALIGN', (1,1), (1,-2), 'LEFT'), # Colonne t ches align e   gauche
                            ('ALIGN', (2,1), (2,-1), 'CENTRE'), # Colonne temps align e au centre
                            ('ALIGN', (0,-1), (1,-1), 'RIGHT'), # Met   droite le mot 'Total :'
        ...
        for index in range(1, len(dictGroupes)+1) :
            ...
            for IDpersonne_ligne, datetime_date, posY, titre_ligne in listeLignes :
                ...
                # R cup ration des pr sences de la ligne
                txtTaches = ""
                txtTemps = ""
        ...
        # Cr ation des totaux de cat gories
        dataTableau = [("", _(u"Total par cat gorie"), ""),]
        totalTemps = 0
        ...
        style = TableStyle([('GRID', (1,0), (2,-2), 0.25, colors.black),
                            ('VALIGN', (0,0), (-1,-1), 'TOP'), # Centre verticalement toutes les cases
                            ('ALIGN', (1,0), (2,0), 'LEFT'), # Titre du groupe   gauche
                            ('ALIGN', (1,1), (1,-1), 'RIGHT'), # Colonne nom align e   droite
                            ('ALIGN', (2,1), (2,-1), 'CENTRE'), # Colonne temps align e au centre
