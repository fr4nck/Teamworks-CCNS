#!/usr/bin/env python3
"""Qualifie le publipostage de contrat et la DPAE/DUE dans l'application Windows."""
...
PATCHED = TEAMWORKS_DIR / "Teamworks_contract_documents_due_smoke.py"
PATCHED_CORE = TEAMWORKS_DIR / "Teamworks_core_contract_documents_due_smoke.py"
REPORT_DIR = ROOT / "artifacts" / "contract-documents-due-smoke"
REPORT = REPORT_DIR / "diagnostic.txt"
MARKER_LINE = '            print("TEAMWORKS_SMOKE_EXAMPLE_READY", flush=True)'
READY_MARKER = "TEAMWORKS_SMOKE_CONTRACT_DOCUMENTS_DUE_READY"
FAILURE_MARKER = "TEAMWORKS_SMOKE_CONTRACT_DOCUMENTS_DUE_FAILED"
...
INJECTION = r'''            print("TEAMWORKS_SMOKE_EXAMPLE_READY", flush=True)
            ...
            try:
                ...
                from Dlg import DLG_Edition_DUE as _smoke_due_legacy
                from Dlg import DLG_Edition_DUE_modern as _smoke_due
                ...
                _smoke_original_due_modal = _smoke_due.Dialog.ShowModal
                _smoke_due.Dialog.ShowModal = lambda self: wx.ID_CANCEL
                try:
                    assert _smoke_registry_dialog.OnBoutonDUE(None)
                finally:
                    _smoke_due.Dialog.ShowModal = _smoke_original_due_modal
                _smoke_registry_dialog.Destroy()
                wx.Yield()

                print("TEAMWORKS_SMOKE_CONTRACT_DOC_STAGE:due-data", flush=True)
                _smoke_due_dialog = _smoke_due.Dialog(frame, IDcontrat=_smoke_contract_id)
                _smoke_due_dialog.Show()
                wx.Yield()
                assert _smoke_due_dialog.due_values["DATE_EMBAUCHE"] == "01092026"
                assert _smoke_due_dialog.due_values["CONTRAT_TYPE"] == "Contrat à durée indéterminée"
                assert _smoke_due_dialog.due_values["DATE_FIN_CONTRAT"] == ""
                assert _smoke_due_dialog.due_values["PERIODE_ESSAI"] == "30"
                assert _smoke_due_dialog.due_values["NATURE_EMPLOI"] == "G1"
                assert _smoke_due_dialog.due_values["DUREE_TRAVAIL_HEBDO"] in ("35", "35.0")

                print("TEAMWORKS_SMOKE_CONTRACT_DOC_STAGE:due-pdf", flush=True)
                _smoke_opened_files = []
                _smoke_original_launcher = _smoke_due_legacy.FonctionsPerso.LanceFichierExterne
                _smoke_due_legacy.FonctionsPerso.LanceFichierExterne = lambda path: _smoke_opened_files.append(path)
                try:
                    _smoke_pdf = _smoke_due.CreationPDF(_smoke_due_dialog, _smoke_due.champs)
                    _smoke_pdf.run()
                    _smoke_pdf_path = _smoke_Path(_smoke_pdf.nomDocument + ".pdf")
                finally:
                    _smoke_due_legacy.FonctionsPerso.LanceFichierExterne = _smoke_original_launcher
                assert _smoke_pdf_path.is_file()
                _smoke_pdf_bytes = _smoke_pdf_path.read_bytes()
                assert _smoke_pdf_bytes.startswith(b"%PDF")
                assert len(_smoke_pdf_bytes) > 5000
                assert _smoke_opened_files and _smoke_opened_files[-1].lower().endswith(".pdf")
                _smoke_due_dialog.Destroy()
                wx.Yield()

                print("TEAMWORKS_SMOKE_CONTRACT_DOCUMENTS_DUE_READY", flush=True)
            except Exception:
                import traceback as _smoke_traceback
                _smoke_traceback.print_exc()
                print("TEAMWORKS_SMOKE_CONTRACT_DOCUMENTS_DUE_FAILED", flush=True)
                wx.CallAfter(self.ExitMainLoop)
                return True
            finally:
                try:
                    if _smoke_pdf_path is not None:
                        _smoke_pdf_path.unlink(missing_ok=True)
...
def build_patched_entrypoint() -> int:
    ...
    if READY_MARKER not in patched_core_source or FAILURE_MARKER not in patched_core_source:
        raise RuntimeError("injection des marqueurs documents contrat/DPAE absente")
    ...
    patched_import = "import Teamworks_core_contract_documents_due_smoke as CORE"
...
def main() -> int:
    ...
    try:
        ...
        if STATICBOX_PARENT_WARNING in output:
            github_error_summary("Contract documents/DPAE StaticBox parentage failed", output)
            return 4
        if return_code != 0 or FAILURE_MARKER in output:
            github_error_summary("Contract documents/DPAE smoke failed", output)
            return return_code or 1
        if READY_MARKER not in output:
            github_error_summary("Contract documents/DPAE smoke failed", output)
            print("marqueur documents contrat/DPAE absent", file=sys.stderr)
            return 2
        return 0
    except Exception:
