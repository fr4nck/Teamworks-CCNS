#!/usr/bin/env python3
"""Construit la fiche individuelle réelle sur les données d'exemple sous Windows."""
...
INJECTION = r'''            print("TEAMWORKS_SMOKE_EXAMPLE_READY", flush=True)
            try:
                ...
                _smoke_dialog = _smoke_person.Dialog(frame, IDpersonne=_smoke_rows[0][0])
                _smoke_dialog.Show()
                wx.Yield()

                print("TEAMWORKS_SMOKE_PERSON_STAGE:assertions", flush=True)
                assert _smoke_dialog.notebook.GetPageCount() == 8
                assert _smoke_dialog.notebook.pageGeneralites is not None
                assert _smoke_dialog.notebook.pageContrats is not None
                assert _smoke_dialog.notebook.pagePresences is not None
                assert _smoke_dialog.notebook.pageCandidatures is not None
                ...
                assert _smoke_dialog.IDpersonne == _smoke_rows[0][0]
