...
INJECTION = r'''            print("TEAMWORKS_SMOKE_EXAMPLE_READY", flush=True)
            try:
                ...
                _selection_ctrl = _selection.ListCtrl(
                    _host,
                    [("ID", "left", 50, "id"), ("Libellé", "left", 160, "label")],
                    [(1, "Alpha"), (2, "Bêta")],
                )
...
def main() -> int:
    ...
    try:
        ...
        if return_code != 0 or FAILURE_MARKER in output:
            github_error_summary("Checklist controls smoke failed", output)
            return return_code or 1
        if READY_MARKER not in output:
            github_error_summary("Checklist controls smoke failed", output)
            print("marqueur des contrôles secondaires absent", file=sys.stderr)
            return 2
        return 0
    except Exception:
